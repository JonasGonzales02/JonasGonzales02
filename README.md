# Hi, I'm Jonas Gonzales 👋

I'm a web development student passionate about building clean, functional websites with a touch of nostalgia. I love combining simple HTML and CSS to create digital experiences that feel both modern and timeless.

---

## 📫 Contact Information

- **Email:** jonas.gonzales@example.edu
- **LinkedIn:** https://linkedin.com/in/jonasgonzales
- **Twitter:** https://twitter.com/jonasgonzales
- **GitHub:** https://github.com/jonasgonzales02

---

## 🌐 Link to My Website

**https://jonasgonzales.com**

---

## 💻 Coding Languages

- HTML5
- CSS3
- JavaScript
- Python
- React
- Node.js

---

## 🚀 Current Projects

- **Starlight Cinema** – A nostalgic vintage theater website with dark mode toggle
- **Leonardo Portfolio** – A clean portfolio site imagining Da Vinci's digital presence
- **Luminary** – A group-built Michelin-starred restaurant website with seasonal design

---

## 🔍 Current Passion Areas / Inquiries

- Vintage and nostalgic web aesthetics
- Responsive design that works everywhere
- Collaborative development and team workflows
- Making complex information easy to navigate
- Accessibility and dark mode implementation
Projects Uploaded to GitHub
The following projects have been uploaded to my GitHub repositories:

Project	Repository Link	Description
Starlight Cinema	https://github.com/jonasgonzales02/Starlight-Cinema	Vintage theater website
Leonardo Portfolio	https://github.com/jonasgonzales02/Leonardo-Portfolio	Artist portfolio site
Luminary	https://github.com/jonasgonzales02/Luminary	Restaurant group project
Submitted to Canvas: [Date]

